...
export class CargosService {
    ...
    async CriarCargo(createCargoDto: CreateCargoDto) {
        ...
        if (cargoExistente) {
            throw new HttpException(
                `O cargo ${cargoExistente.cargo.toUpperCase()} já esta cadastrado no sistema.`,
    ...
    async ListarCargos() {
        ...
        if (!cargosCadastrados) {
            throw new HttpException(
                'Não existe nenhum cargo cadastrado no sistema.',
    ...
    async BuscarCargoID(id: number) {
        ...
        throw new HttpException(
            'O ID informado não esta vindulado a nenhum cargo cadastrado no sistema.',
            HttpStatus.NOT_FOUND
        );
    }
    ...
    async AtualizarCargo(id: number, updateCargoDto: UpdateCargoDto) {
        ...
        // }
        try {
            const idCargo = await this.prisma.cargos.findFirst({
                where: { id }
            });
